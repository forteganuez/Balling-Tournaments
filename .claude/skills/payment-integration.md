...
### Stripe-Specific (this project uses Stripe)

- All amounts in **cents** (integers) — never in dollars.
- Use `entryFee = 0` to skip Stripe entirely and register directly.
- Stripe Checkout for payment flows, webhooks for async confirmation.
- Store `stripeCustomerId`, `stripePaymentIntentId` in DB for reconciliation.

## Common Failures

- Payment processor collapse during traffic spike → webhook queue backups, revenue loss
- Out-of-order webhooks breaking handlers (no idempotency) → production failures
